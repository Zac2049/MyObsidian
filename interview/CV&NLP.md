...
LSTM 单元包括三个门（gate）：输入门（input gate）、遗忘门（forget gate）、输出门（output gate）。每个门都由一个 sigmoid 激活函数和一个乘法操作组成。LSTM 的隐藏状态 $h_t$ 和细胞状态（cell state） $c_t$ 可以通过以下公式计算：
...
$$
c_t = f_t \cdot c_{t-1} + i_t \cdot \text{tanh}(W_{xc}x_t + W_{hc}h_{t-1} + b_c)
$$
